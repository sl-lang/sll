...
if [ ! -d "build/_sll_runtime_$platform" ]; then
	...
	curl -o "build/_sll_runtime_$platform/$platform.zip" "https://sl-lang.github.io/bin/nightly/$platform.zip"
...
if [[ ! -f "build/_build_script.sla" || "$SLL_FORCE_REBUILD" != "" ]]; then
	"build/_sll_runtime_$platform/sll" src/_build/main.sll -I "@build-script|src/_build" -r -R -a -o build/_build_script
fi
