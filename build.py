...
if (os.name=="nt"):
	...
	if ("--run" in sys.argv):
		...
		if (subprocess.run(["lll.exe","../example/test.lll","-v","-O0","-c","-o","test","-p","-e","-I","../example","-m","-R"]).returncode!=0 or subprocess.run(["lll.exe","test.lllc","-v","-O3","-p","-P","-e","-L","-a","-c","-o","test2"]).returncode!=0):
			os.chdir(cd)
			sys.exit(1)
		os.chdir(cd)
else:
	...
	if ("--run" in sys.argv):
		...
		if (subprocess.run(["build/lll","example/test.lll","-v","-O0","-c","-o","build/test","-p","-e","-I","example","-m","-R"]).returncode!=0 or subprocess.run(["build/lll","build/test.lllc","-v","-O3","-p","-P","-e","-L","-a","-c","-o","build/test2"]).returncode!=0):
			sys.exit(1)
