...
static sll_object_t* _optimize(sll_object_t* o,sll_object_t* p,optimizer_data_t* o_dt,uint8_t fl){
	...
	if (o_dt->vi!=SLL_MAX_VARIABLE_INDEX){
		...
		*(o_dt->v+o_dt->vi)=_get_as_runtime_object(o,o_dt,1);
		...
		case SLL_OBJECT_TYPE_PRINT:
			{
				...
				while (l){
					...
					sll_runtime_object_t* v=_get_as_runtime_object(b,o_dt,0);
					if (SLL_RUNTIME_OBJECT_GET_TYPE(v)!=RUNTIME_OBJECT_TYPE_UNKNOWN&&!(v->t&RUNTIME_OBJECT_CHANGE_IN_LOOP)){
						...
						if (b->t!=SLL_OBJECT_TYPE_STRING&&b->t!=SLL_OBJECT_TYPE_CHAR){
							_shift_objects(b,o_dt->c_dt,1);
							b->t=SLL_OBJECT_TYPE_COMMA;
							b->dt.ac=2;
							if (o->t==OBJECT_TYPE_CHANGE_STACK){
								o=o->dt._p;
							}
		...
		case SLL_OBJECT_TYPE_ASSIGN:
			{
				...
				if (SLL_IDENTIFIER_GET_ARRAY_ID(id_o->dt.id)==SLL_MAX_SHORT_IDENTIFIER_LENGTH){
					o_dt->vi=(o_dt->it.l_im+SLL_IDENTIFIER_GET_ARRAY_INDEX(id_o->dt.id))->v;
					(*(o_dt->va.l_sm+(SLL_IDENTIFIER_GET_ARRAY_INDEX(id_o->dt.id)>>6)))|=1ull<<(SLL_IDENTIFIER_GET_ARRAY_INDEX(id_o->dt.id)&63);
				}
				else{
					o_dt->vi=(o_dt->it.s_im[SLL_IDENTIFIER_GET_ARRAY_ID(id_o->dt.id)]+SLL_IDENTIFIER_GET_ARRAY_INDEX(id_o->dt.id))->v;
		...
		case SLL_OBJECT_TYPE_SWITCH:
			{
				...
				l=(l-1)>>1;
				sll_integer_list_t il=SLL_ZERO_INTEGER_LIST_STRUCT;
				do{
					l--;
					sll_object_t* a=o;
					o=_optimize(o,r,o_dt,fl|OPTIMIZER_FLAG_ARGUMENT);
...
_optimize_operation_list_comma:
			{
				...
				do{
					...
					if (o->t==SLL_OBJECT_TYPE_PRINT){
						...
						if (a){
							o=_optimize(a,r,o_dt,fl&OPTIMIZER_FLAG_IGNORE_LOOP_FLAG);
						}
						a=NULL;
					}
					o=sll_skip_object(o);
				...
				o_dt->rm=rm_fn;
				if (a){
					o=_optimize(a,r,o_dt,(fl&OPTIMIZER_FLAG_IGNORE_LOOP_FLAG)|(!rm&&r->t==SLL_OBJECT_TYPE_COMMA?OPTIMIZER_FLAG_ARGUMENT:0));
				}
				if ((p&&(p->t==SLL_OBJECT_TYPE_FUNC||p->t==SLL_OBJECT_TYPE_INLINE_FUNC||p->t==SLL_OBJECT_TYPE_COMMA||p->t==SLL_OBJECT_TYPE_OPERATION_LIST))){
