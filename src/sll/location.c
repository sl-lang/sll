...
__SLL_EXTERNAL sll_file_offset_t sll_get_location(const sll_assembly_data_t* assembly_data,sll_instruction_index_t instruction_index,sll_string_index_t* file_path_string_index,sll_string_index_t* function_string_index){
	sll_string_index_t file_path=(assembly_data->string_table.length?0:SLL_MAX_STRING_INDEX);
	if (instruction_index>=assembly_data->instruction_count){
		if (file_path_string_index){
			*file_path_string_index=file_path;
		}
	...
	sll_instruction_index_t counter=0;
	sll_file_offset_t line=0;
	sll_string_index_t function=SLL_MAX_STRING_INDEX;
	for (sll_debug_data_length_t i=0;i<assembly_data->debug_data.length;i++){
		counter+=(assembly_data->debug_data.data+i)->delta_instruction_index;
		if (counter>instruction_index){
			break;
		}
		if ((assembly_data->debug_data.data+i)->line&SLL_DEBUG_LINE_DATA_FLAG_FILE){
			file_path=(sll_string_index_t)SLL_DEBUG_LINE_DATA_GET_DATA(assembly_data->debug_data.data+i);
			line=0;
		}
		else if ((assembly_data->debug_data.data+i)->line&SLL_DEBUG_LINE_DATA_FLAG_FUNC){
			function=(sll_string_index_t)SLL_DEBUG_LINE_DATA_GET_DATA(assembly_data->debug_data.data+i);
		}
		else{
			line=SLL_DEBUG_LINE_DATA_GET_DATA(assembly_data->debug_data.data+i);
		}
	}
	if (file_path_string_index){
		*file_path_string_index=file_path;
	...
	if (function_string_index){
		*function_string_index=function;
	}
	return line+1;
}
