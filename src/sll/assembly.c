...
static sll_assembly_instruction_t* _get_previous_instruction(assembly_generator_data_t* g_dt){
	...
	do{
		...
		o=(SLL_ASSEMBLY_INSTRUCTION_GET_TYPE(o)==SLL_ASSEMBLY_INSTRUCTION_TYPE_CHANGE_STACK?o->dt._p:o-1);
		i--;
	} while (SLL_ASSEMBLY_INSTRUCTION_GET_TYPE(o)==ASSEMBLY_INSTRUCTION_TYPE_NOP||SLL_ASSEMBLY_INSTRUCTION_GET_TYPE(o)==ASSEMBLY_INSTRUCTION_TYPE_FUNC_START||SLL_ASSEMBLY_INSTRUCTION_GET_TYPE(o)==ASSEMBLY_INSTRUCTION_TYPE_LABEL_TARGET||SLL_ASSEMBLY_INSTRUCTION_GET_TYPE(o)==SLL_ASSEMBLY_INSTRUCTION_TYPE_CHANGE_STACK);
	return o;
}
		...
		case SLL_NODE_TYPE_IDENTIFIER:
			{
				...
				if (SLL_IDENTIFIER_GET_ARRAY_ID(o->dt.id)==SLL_MAX_SHORT_IDENTIFIER_LENGTH){
					...
					sll_identifier_t* id=g_dt->c_dt->idt.s[SLL_IDENTIFIER_GET_ARRAY_ID(o->dt.id)].dt+i;
		...
		case SLL_NODE_TYPE_ASSIGN:
			{
				...
				if (SLL_IDENTIFIER_GET_ARRAY_ID(io->dt.id)==SLL_MAX_SHORT_IDENTIFIER_LENGTH){
					if (*(g_dt->rm.l+SLL_IDENTIFIER_GET_ARRAY_INDEX(io->dt.id))==io){
						ai->t=ASSEMBLY_INSTRUCTION_TYPE_NOP;
					}
					else{
						ai->dt.v=(g_dt->it.l_im+SLL_IDENTIFIER_GET_ARRAY_INDEX(io->dt.id))->v;
					}
				}
				else{
					if (*(g_dt->rm.s[SLL_IDENTIFIER_GET_ARRAY_ID(io->dt.id)]+SLL_IDENTIFIER_GET_ARRAY_INDEX(io->dt.id))==io){
						ai->t=ASSEMBLY_INSTRUCTION_TYPE_NOP;
					}
					else{
						ai->dt.v=(g_dt->it.s_im[SLL_IDENTIFIER_GET_ARRAY_ID(io->dt.id)]+SLL_IDENTIFIER_GET_ARRAY_INDEX(io->dt.id))->v;
					}
				}
				l-=2;
				while (l){
		...
		case SLL_NODE_TYPE_INLINE_IF:
			{
				...
				while (l){
					...
					if (p_ai->t!=SLL_ASSEMBLY_INSTRUCTION_TYPE_RET){
		...
		case SLL_NODE_TYPE_WHILE_ARRAY:
			{
				...
				o=(o->t==SLL_NODE_TYPE_FOR_ARRAY?_generate_jump(cnd,g_dt,e,1):sll_skip_node_const(cnd));
				DEFINE_LABEL(g_dt,s);
				l-=2;
				while (l){
					l--;
		...
		case SLL_NODE_TYPE_WHILE_MAP:
			{
				...
				assembly_instruction_label_t e=NEXT_LABEL(g_dt);
				const sll_node_t* cnd=_generate_loop_start(g_dt,o,&lg_dt,c,e);
				o=(o->t==SLL_NODE_TYPE_FOR_MAP?_generate_jump(cnd,g_dt,e,1):sll_skip_node_const(cnd));
				DEFINE_LABEL(g_dt,s);
				l-=2;
				sll_bool_t extra=(l==1);
				while (l){
					l--;
					if (l<2){
						o=_generate_on_stack(o,g_dt);
		...
		case SLL_NODE_TYPE_WHILE:
			{
				...
				assembly_instruction_label_t e=NEXT_LABEL(g_dt);
				assembly_loop_generator_data_t lg_dt;
				const sll_node_t* cnd=sll_skip_node_const(_generate_loop_start(g_dt,o,&lg_dt,c,e));
				DEFINE_LABEL(g_dt,s);
				while (l){
					l--;
					o=_generate(o,g_dt);
