...
#define GET_VARIABLE_INDEX(o,g_dt) (((g_dt)->identifier_map.data+(o)->data.identifier_index)->variable)
...
static struct _SLL_ASSEMBLY_INSTRUCTION _assembly_nop={
	ASSEMBLY_INSTRUCTION_TYPE_NOP
};



static sll_node_t _generate_on_stack(sll_node_t o,assembly_generator_data_t* g_dt);



static sll_node_t _generate(sll_node_t o,assembly_generator_data_t* g_dt);
		...
		case SLL_NODE_TYPE_IDENTIFIER:
			{
				...
				if ((g_dt->identifier_map.data+o->data.identifier_index)->variable==SLL_MAX_VARIABLE_INDEX){
					if (SLL_IDENTIFIER_IS_TLS(id)){
						...
						(g_dt->identifier_map.data+o->data.identifier_index)->variable=g_dt->identifier_map.variable_count<<1;
						g_dt->identifier_map.variable_count++;
					}
				}
				if (*(g_dt->identifier_remove_data+o->data.identifier_index)!=VARIABLE_OFFSET_NEVER_DELETE){
					if (fn_sc==SLL_MAX_SCOPE||id->scope>=fn_sc){
						*(g_dt->identifier_remove_data+o->data.identifier_index)=PTR(o);
					}
					else{
						*(g_dt->identifier_remove_data+o->data.identifier_index)=VARIABLE_OFFSET_NEVER_DELETE;
					}
				}
				return o+1;
			}
...
static sll_node_t _skip_with_dbg(sll_node_t o,assembly_generator_data_t* g_dt){
	while (o->type==SLL_NODE_TYPE_NOP||o->type==SLL_NODE_TYPE_DBG||o->type==SLL_NODE_TYPE_CHANGE_STACK){
		GENERATE_DEBUG_DATA(g_dt,o);
		o=(o->type==SLL_NODE_TYPE_CHANGE_STACK?o->data._next_node:o+1);
	}
	if (!SLL_NODE_HAS_CHILDREN(o)){
		return o+1;
	}
	sll_arg_count_t l=o->data.arg_count;
	o++;
...
static sll_node_t _generate_sequential_jump(sll_node_t o,assembly_generator_data_t* g_dt,sll_assembly_instruction_type_t t,sll_bool_t stack){
	sll_arg_count_t l=o->data.arg_count;
	o++;
	if (!l){
		if (stack){
...
static sll_node_t _generate_cond_jump(sll_node_t o,assembly_generator_data_t* g_dt,assembly_instruction_label_t lbl,sll_assembly_instruction_type_t t){
	sll_arg_count_t l=o->data.arg_count;
	o++;
	if (!l){
		return o;
...
static void _generate_identifier(sll_node_t n,assembly_generator_data_t* g_dt){
	sll_assembly_instruction_t ai=_acquire_next_instruction(g_dt->assembly_data);
	ai->type=SLL_ASSEMBLY_INSTRUCTION_TYPE_LOAD;
	if (*(g_dt->identifier_remove_data+n->data.identifier_index)==n){
		ai->type=SLL_ASSEMBLY_INSTRUCTION_TYPE_LOAD_DEL;
	}
	ai->data.variable=(g_dt->identifier_map.data+n->data.identifier_index)->variable;
	PUSH;
}



static sll_node_t _generate_two_on_stack(sll_node_t o,assembly_generator_data_t* g_dt,sll_arg_count_t l){
	SLL_ASSERT(l);
	o=_generate_on_stack(o,g_dt);
	l--;
	if (!l){
...
static sll_node_t _generate_jump(sll_node_t o,assembly_generator_data_t* g_dt,assembly_instruction_label_t lbl,sll_bool_t inv){
	while (o->type==SLL_NODE_TYPE_NOP||o->type==SLL_NODE_TYPE_DBG||o->type==SLL_NODE_TYPE_CHANGE_STACK){
		GENERATE_DEBUG_DATA(g_dt,o);
		o=(o->type==SLL_NODE_TYPE_CHANGE_STACK?o->data._next_node:o+1);
	}
...
static sll_node_t _generate_call(sll_node_t o,assembly_generator_data_t* g_dt){
...
static sll_node_t _generate_call_array(sll_node_t o,assembly_generator_data_t* g_dt){
...
static sll_node_t _generate_inline_function(sll_node_t o,assembly_generator_data_t* g_dt){
...
static sll_node_t _generate_assign(sll_node_t o,assembly_generator_data_t* g_dt){
	sll_arg_count_t l=o->data.arg_count;
	o++;
	if (!l){
		return o;
	...
	if (io->type==SLL_NODE_TYPE_IDENTIFIER){
		if (*(g_dt->identifier_remove_data+io->data.identifier_index)==io){
			o++;
			l--;
			do{
				o=_generate(o,g_dt);
...
static sll_node_t _mark_loop_delete(sll_node_t o,const assembly_generator_data_t* g_dt,bitmap_t* v_st,sll_scope_t sc){
	while (o->type==SLL_NODE_TYPE_NOP||o->type==SLL_NODE_TYPE_DBG||o->type==SLL_NODE_TYPE_CHANGE_STACK){
		o=(o->type==SLL_NODE_TYPE_CHANGE_STACK?o->data._next_node:o+1);
	}
	switch (o->type){
		...
		case SLL_NODE_TYPE_IDENTIFIER:
			{
				const sll_identifier_t* id=g_dt->source_file->identifier_table.data+o->data.identifier_index;
				if (id->scope<=sc&&*(g_dt->identifier_remove_data+o->data.identifier_index)==o){
					*(g_dt->identifier_remove_data+o->data.identifier_index)=VARIABLE_OFFSET_NEVER_DELETE;
					sll_variable_index_t k=(g_dt->identifier_map.data+o->data.identifier_index)->variable;
					*(v_st+(k>>6))|=1ull<<(k&63);
				}
				return o+1;
			}
...
static sll_node_t _generate_loop_start(assembly_generator_data_t* g_dt,sll_node_t o,assembly_loop_generator_data_t* lg_dt,assembly_instruction_label_t count,assembly_instruction_label_t brk){
		...
		case SLL_NODE_TYPE_WHILE_MAP:
			{
				...
				assembly_instruction_label_t e=NEXT_LABEL(g_dt);
				sll_node_t cnd=_generate_loop_start(g_dt,o,&lg_dt,c,e);
				sll_node_t r=o;
				o=(r->type==SLL_NODE_TYPE_FOR_ARRAY||r->type==SLL_NODE_TYPE_FOR_MAP?_generate_jump(cnd,g_dt,e,1):sll_skip_node(cnd));
				DEFINE_LABEL(g_dt,s);
				l-=2;
...
static sll_node_t _generate(sll_node_t o,assembly_generator_data_t* g_dt){
	...
	switch (o->type){
		...
		case SLL_NODE_TYPE_IDENTIFIER:
			{
				if (*(g_dt->identifier_remove_data+o->data.identifier_index)==o){
					sll_assembly_instruction_t ai=_acquire_next_instruction(g_dt->assembly_data);
					ai->type=SLL_ASSEMBLY_INSTRUCTION_TYPE_DEL;
					ai->data.variable=(g_dt->identifier_map.data+o->data.identifier_index)->variable;
				}
				return o+1;
			}
		case SLL_NODE_TYPE_PRINT:
		...
		case SLL_NODE_TYPE_ASSIGN:
			return _generate_assign(o,g_dt);
		case SLL_NODE_TYPE_FUNC:
			{
				sll_assembly_instruction_t ai=_acquire_next_instruction(g_dt->assembly_data);
				ai->type=ASSEMBLY_INSTRUCTION_TYPE_DBG_FUNC;
		...
		case SLL_NODE_TYPE_WHILE_MAP:
			{
				...
				assembly_loop_generator_data_t lg_dt;
				sll_node_t cnd=_generate_loop_start(g_dt,o,&lg_dt,c,e);
				o=(o->type>=SLL_NODE_TYPE_WHILE?sll_skip_node(cnd):_generate_jump(cnd,g_dt,e,1));
				DEFINE_LABEL(g_dt,s);
				l-=2;
				while (l){
					l--;
...
__SLL_EXTERNAL void sll_generate_assembly(const sll_source_file_t* source_file,sll_assembly_data_t* out){
	...
	assembly_generator_data_t g_dt={
		...
		source_file,
		{
			.data=sll_allocate(source_file->identifier_table.length*sizeof(identifier_data_t)),
			.variable_count=0,
			.tls_variable_count=0
		},
		0,
		sll_allocate(source_file->identifier_table.length*sizeof(void*)),
		{
	...
	for (sll_identifier_table_length_t i=0;i<source_file->identifier_table.length;i++){
		(g_dt.identifier_map.data+i)->variable=SLL_MAX_VARIABLE_INDEX;
		*(g_dt.identifier_remove_data+i)=NULL;
	}
	...
	for (sll_function_index_t i=0;i<source_file->function_table.length;i++){
		...
		for (sll_arg_count_t j=0;j<SLL_FUNCTION_GET_ARGUMENT_COUNT(k);j++){
			...
			(g_dt.identifier_map.data+ii)->variable=g_dt.identifier_map.variable_count<<1;
			g_dt.identifier_map.variable_count++;
		}
		g_dt.stack_offset=0;
		sll_node_t fo=_get_node_at_offset(source_file,k->offset);
	...
	sll_deallocate(g_dt.identifier_map.data);
	sll_deallocate(g_dt.identifier_remove_data);
