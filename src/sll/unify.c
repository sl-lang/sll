		...
		if (src->type==SLL_NODE_TYPE_CHANGE_STACK){
			...
			*dst=*src;
			if (sf_m_dt&&src->type==SLL_NODE_TYPE_DBG&&src->data.string_index!=SLL_MAX_STRING_INDEX){
				dst->data.string_index=*(sf_m_dt->string_map+src->data.string_index);
			}
			src++;
		}
	}
	...
	*dst=*src;
	switch (src->type){
		...
		case SLL_NODE_TYPE_FIELD:
			if (sf_m_dt){
				dst->data.string_index=*(sf_m_dt->string_map+dst->data.string_index);
			}
			return src+1;
		case SLL_NODE_TYPE_IDENTIFIER:
			if (sf_m_dt){
				dst->data.identifier_index=*(sf_m_dt->identifier_index_offset[SLL_IDENTIFIER_GET_ARRAY_ID(dst->data.identifier_index)]+SLL_IDENTIFIER_GET_ARRAY_INDEX(dst->data.identifier_index));
			}
			return src+1;
		...
		case SLL_NODE_TYPE_FUNC:
			if (sf_m_dt){
				dst->data.function.function_index+=sf_m_dt->function_index_offset;
				dst->data.function.scope+=sf_m_dt->scope_offset;
			}
		...
		case SLL_NODE_TYPE_WHILE_MAP:
			{
				if (sf_m_dt){
					dst->data.loop.scope+=sf_m_dt->scope_offset;
				}
		...
		case SLL_NODE_TYPE_DECL:
			{
				if (sf_m_dt&&dst->data.declaration.name_string_index!=SLL_MAX_STRING_INDEX){
					dst->data.declaration.name_string_index=*(sf_m_dt->string_map+dst->data.declaration.name_string_index);
				}
	...
	do{
		...
		if (!o->first_node->data.arg_count){
			...
			for (sll_string_index_t i=0;i<sf->string_table.length;i++){
				...
				*(sf_m_dt.string_map+i)=sll_add_string(&(o->string_table),&tmp,1);
			}
			sll_node_t* dbg=_acquire_next_node(o);
			dbg->type=SLL_NODE_TYPE_DBG;
			dbg->data.string_index=*(sf_m_dt.string_map+sf->file_path_string_index);
			if (!idx){
			...
			for (unsigned int i=0;i<SLL_MAX_SHORT_IDENTIFIER_LENGTH;i++){
				const sll_identifier_list_t* il=sf->identifier_table.short_+i;
				if (!il->length){
					sf_m_dt.identifier_index_offset[i]=NULL;
					continue;
				}
				sll_identifier_list_t* oil=o->identifier_table.short_+i;
				sll_identifier_list_length_t off=oil->length;
				sf_m_dt.identifier_index_offset[i]=sll_allocate_stack(il->length*sizeof(sll_identifier_index_t));
				oil->length+=il->length;
				oil->data=sll_reallocate(oil->data,oil->length*sizeof(sll_identifier_t));
				for (sll_identifier_list_length_t j=0;j<il->length;j++){
					*(sf_m_dt.identifier_index_offset[i]+j)=SLL_CREATE_IDENTIFIER(off+j,i);
					(oil->data+off+j)->scope=(il->data+j)->scope+o->_next_scope;
					SLL_IDENTIFIER_SET_STRING_INDEX(oil->data+off+j,*(sf_m_dt.string_map+SLL_IDENTIFIER_GET_STRING_INDEX(il->data+j)),SLL_IDENTIFIER_IS_TLS(il->data+j));
				}
			}
			if (sf->identifier_table.long_data_length){
				sll_identifier_list_length_t off=o->identifier_table.long_data_length;
				sf_m_dt.identifier_index_offset[SLL_MAX_SHORT_IDENTIFIER_LENGTH]=sll_allocate_stack(sf->identifier_table.long_data_length*sizeof(sll_identifier_index_t));
				o->identifier_table.long_data_length+=sf->identifier_table.long_data_length;
				...
				for (sll_identifier_list_length_t i=0;i<sf->identifier_table.long_data_length;i++){
					*(sf_m_dt.identifier_index_offset[SLL_MAX_SHORT_IDENTIFIER_LENGTH]+i)=SLL_CREATE_IDENTIFIER(off+i,SLL_MAX_SHORT_IDENTIFIER_LENGTH);
					(o->identifier_table.long_data+off+i)->scope=(sf->identifier_table.long_data+i)->scope+o->_next_scope;
					SLL_IDENTIFIER_SET_STRING_INDEX(o->identifier_table.long_data+off+i,*(sf_m_dt.string_map+SLL_IDENTIFIER_GET_STRING_INDEX(sf->identifier_table.long_data+i)),SLL_IDENTIFIER_IS_TLS(sf->identifier_table.long_data+i));
				}
			}
			else{
				sf_m_dt.identifier_index_offset[SLL_MAX_SHORT_IDENTIFIER_LENGTH]=NULL;
			}
			for (sll_import_index_t i=0;i<sf->import_table.length;i++){
				...
				for (sll_identifier_list_length_t j=0;j<k->length;j++){
					*(sf_m_dt.identifier_index_offset[SLL_IDENTIFIER_GET_ARRAY_ID(k->data[j])]+SLL_IDENTIFIER_GET_ARRAY_INDEX(k->data[j]))=*(ex_dt+j);
				}
			}
			if (idx){
				sll_identifier_index_t* ex_dt=sll_allocate_stack(sf->export_table.length*sizeof(sll_identifier_index_t));
				*(export_dt+idx-1)=ex_dt;
				for (sll_export_table_length_t i=0;i<sf->export_table.length;i++){
					*(ex_dt+i)=*(sf_m_dt.identifier_index_offset[SLL_IDENTIFIER_GET_ARRAY_ID(*(sf->export_table.data+i))]+SLL_IDENTIFIER_GET_ARRAY_INDEX(*(sf->export_table.data+i)));
				}
			}
			if (sf->function_table.length){
				...
				for (sll_function_index_t i=0;i<sf->function_table.length;i++){
					const sll_function_t* s=*(sf->function_table.data+i);
					sll_function_t* d=sll_allocate(sizeof(sll_function_t)+s->arg_count*sizeof(sll_identifier_index_t));
					*(o->function_table.data+sf_m_dt.function_index_offset+i)=d;
					d->offset=s->offset+o->_stack.offset;
					d->name_string_index=(s->name_string_index==SLL_MAX_STRING_INDEX?SLL_MAX_STRING_INDEX:*(sf_m_dt.string_map+s->name_string_index));
					d->description_string_index=(s->description_string_index==SLL_MAX_STRING_INDEX?SLL_MAX_STRING_INDEX:*(sf_m_dt.string_map+s->description_string_index));
					d->arg_count=s->arg_count;
					for (sll_arg_count_t j=0;j<SLL_FUNCTION_GET_ARGUMENT_COUNT(s);j++){
						d->args[j]=*(sf_m_dt.identifier_index_offset[SLL_IDENTIFIER_GET_ARRAY_ID(s->args[j])]+SLL_IDENTIFIER_GET_ARRAY_INDEX(s->args[j]));
					}
				}
			}
			_clone_node(sf->first_node,o,&sf_m_dt);
			for (unsigned int i=0;i<=SLL_MAX_SHORT_IDENTIFIER_LENGTH;i++){
				sll_deallocate(sf_m_dt.identifier_index_offset[i]);
			}
			sll_deallocate(sf_m_dt.string_map);
		}
