...
__SLL_EXTERNAL __SLL_CHECK_OUTPUT sll_object_t* sll_operator_sub(sll_object_t* a,sll_object_t* b){
	if (a->t==SLL_OBJECT_TYPE_ARRAY){
		...
		if (b->t==SLL_OBJECT_TYPE_ARRAY){
			sll_array_remove_multiple(&(a->dt.a),b->dt.a.data,b->dt.a.length,&(o->dt.a));
		}
		else{
		...
		case COMBINED_TYPE_CS:
			{
				...
				for (sll_array_length_t i=0;i<b->dt.s.l;i++){
					o->dt.a.data[i]=sll_operator_sub(a,sll_static_char[b->dt.s.v[i]]);
				}
				return o;
			}
		...
		case COMBINED_TYPE_CA:
			{
				...
				for (sll_array_length_t i=0;i<b->dt.a.length;i++){
					o->dt.a.data[i]=sll_operator_sub(a,b->dt.a.data[i]);
				}
				return o;
			}
		...
		case COMBINED_TYPE_CM:
			{
				...
				for (sll_array_length_t i=0;i<(m.length<<1);i+=2){
					...
					o->dt.m.data[i+1]=sll_operator_sub(a,m.data[i+1]);
				}
				return o;
			}
		...
		case COMBINED_TYPE_SC:
			{
				...
				for (sll_string_length_t i=0;i<a->dt.s.l;i++){
					o->dt.a.data[i]=sll_operator_sub(sll_static_char[a->dt.s.v[i]],b);
				}
				return o;
			}
