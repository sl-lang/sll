...
__SLL_EXTERNAL __SLL_CHECK_OUTPUT sll_compare_result_t sll_operator_compare(sll_object_t* a,sll_object_t* b){
	switch (COMBINED_ARGS){
		...
		case COMBINED_TYPE_ID:
			return COMPARE_RESULT(a->dt.i,COMPLEX_ABS(b->dt.d));
		...
		case COMBINED_TYPE_FC:
			return COMPARE_RESULT(a->dt.f,b->dt.c);
		case COMBINED_TYPE_FD:
			return COMPARE_RESULT(a->dt.f,COMPLEX_ABS(b->dt.d));
		...
		case COMBINED_TYPE_CD:
			return COMPARE_RESULT(a->dt.c,COMPLEX_ABS(b->dt.d));
		...
		case COMBINED_TYPE_DI:
			return COMPARE_RESULT(COMPLEX_ABS(a->dt.d),b->dt.i);
		case COMBINED_TYPE_DF:
			return COMPARE_RESULT(COMPLEX_ABS(a->dt.d),b->dt.f);
		case COMBINED_TYPE_DC:
			return COMPARE_RESULT(COMPLEX_ABS(a->dt.d),b->dt.c);
		case COMBINED_TYPE_DD:
			return COMPARE_RESULT(COMPLEX_ABS(a->dt.d),COMPLEX_ABS(b->dt.d));
		...
		case COMBINED_TYPE_SF:
			return COMPARE_RESULT(a->dt.s.l,b->dt.f);
		...
		case COMBINED_TYPE_SD:
			return COMPARE_RESULT(a->dt.s.l,COMPLEX_ABS(b->dt.d));
