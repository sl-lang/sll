...
__SLL_EXTERNAL __SLL_API_CALL void sll_api_path_relative(const sll_string_t* path,const sll_string_t* base,sll_string_t* out){
	...
	sll_path_relative(path->data,base->data,out);
}
...
__SLL_EXTERNAL void sll_path_relative(const sll_char_t* s,const sll_char_t* b,sll_string_t* o){
	...
	if (s_drv!=b_drv||(s_drv==2&&_path_norm_drive[ns.data[0]]!=_path_norm_drive[nb.data[0]])||(s_drv>2&&sll_compare_data(ns.data,nb.data,s_drv)!=SLL_COMPARE_RESULT_EQUAL)){
		*o=ns;
		sll_free_string(&nb);
		return;
	}
