...
__API_FUNC(time_sleep){
	...
	sll_platform_sleep((SLL_OBJECT_GET_TYPE(a)==SLL_OBJECT_TYPE_INT?a->dt.i*1000000000:SLL_FLOAT_ROUND(a->dt.f*1e9)));
...
__API_FUNC(time_sleep_nanos){
	sll_time_t st=sll_platform_get_current_time();
	sll_platform_sleep((SLL_OBJECT_GET_TYPE(a)==SLL_OBJECT_TYPE_INT?a->dt.i:SLL_FLOAT_ROUND(a->dt.f)));
