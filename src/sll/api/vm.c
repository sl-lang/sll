...
__SLL_EXTERNAL __SLL_CHECK_OUTPUT sll_object_t* sll_instruction_to_location(sll_instruction_index_t ii){
	...
	return sll_new_object(SLL_CHAR("(sis)"),sll_current_runtime_data->a_dt->string_table.dt+fp_i,ln,(fn_i==SLL_MAX_STRING_INDEX?&_vm_code_name:sll_current_runtime_data->a_dt->string_table.dt+fn_i));
}
