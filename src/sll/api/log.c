...
__SLL_EXTERNAL __SLL_API_CALL __SLL_CHECK_OUTPUT sll_bool_t sll_api_log_log(sll_object_t*const* data,sll_arg_count_t len){
	...
	sll_bool_t o=sll_log_raw((sll_current_runtime_data->a_dt->st.dt+fp)->data,(fn==SLL_MAX_STRING_INDEX?SLL_CHAR("@code"):(sll_current_runtime_data->a_dt->st.dt+fn)->data),ln,0,&s);
	sll_free_string(&s);
	return o;
}
