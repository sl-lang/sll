...
#include <window_manager/event.h>
...
__WINDOW_MANAGER_API_CALL void window_api_event_poll(sll_bool_t blocking,sll_array_t* out){
	SLL_INIT_ARRAY(out);
			...
			case XCB_MOTION_NOTIFY:
				{
					...
					arg=sll_new_object(SLL_CHAR("uuhh"),WINDOW_EVENT_MOUSE,motion_event->event,motion_event->event_x,motion_event->event_y);
					break;
				}
			case XCB_LEAVE_NOTIFY:
				{
					const xcb_leave_notify_event_t* leave_event=(const xcb_leave_notify_event_t*)event;
					arg=sll_new_object(SLL_CHAR("uu"),WINDOW_EVENT_MOUSE_LEAVE,leave_event->event);
					break;
				}
			case XCB_FOCUS_IN:
			case XCB_FOCUS_OUT:
