...
__WINDOW_API_CALL void window_api_display_enumerate(sll_array_t* out){
	...
	for (uint32_t i=0;i<count;i++){
		...
		out->data[i]=sll_new_object(SLL_CHAR("uhhuufi"),id,(int32_t)(rect.origin.x),(int32_t)(rect.origin.y),(uint32_t)(rect.size.width),(uint32_t)(rect.size.height),CGDisplayRotation(id),flags);
	}
	sll_deallocate(data);
}
