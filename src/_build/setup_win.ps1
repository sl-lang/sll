...
$vcvarsall = (& "${Env:ProgramFiles(x86)}\Microsoft Visual Studio\Installer\vswhere.exe" -products '*' -latest -prerelease -property installationPath) + "\VC\Auxiliary\Build\vcvarsall.bat"
foreach ($elem in @(& "$vcvarsall" x64 "&&" set)) {
	$elem=$elem.Split("=")
	if ($elem[0] -eq "PATH"){
		$PATH=$elem[1]
	}
