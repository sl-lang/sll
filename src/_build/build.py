		...
		if (util.release):
			...
			for k in fl:
				...
				if (hashlist.update(k,"src/sll/include")):
					util.log("    "+k)
					if (k[-2:]==".c"):
						if (util.execute(["gcc","-fno-exceptions","-fno-stack-protector","-fdiagnostics-color=always","-fPIC","-c","-fvisibility=hidden","-Wall","-O3","-Werror","-I","src/sll/include","-o",out_fp,k]+linux_opt)):
							hashlist.fail(k)
							err=True
