...
def build_sll(fl,v,r):
	...
	if (util.system=="windows"):
		...
		if (r):
			...
			if (util.execute(["link",f"/OUT:build/{lib_nm}.dll","/DLL","/DYNAMICBASE","/MACHINE:X64","/ERRORREPORT:none","/NOLOGO","/TLBID:1","/WX","/LTCG","/OPT:NOREF","/INCREMENTAL:NO","/RELEASE","bcrypt.lib"]+out_fl)):
				sys.exit(1)
		else:
			...
			if (util.execute(["link",f"/OUT:build/{lib_nm}.dll","/DLL","/DYNAMICBASE","/MACHINE:X64","/ERRORREPORT:none","/NOLOGO","/TLBID:1","/WX","/DEBUG","/INCREMENTAL:NO","/RELEASE","/PDB:build/sll.pdb","bcrypt.lib"]+out_fl)):
				sys.exit(1)
	else:
...
def build_sll_extension(nm,fl,v,r):
	...
	if (util.system=="windows"):
		if (r):
			...
			if (util.execute(["link",f"/OUT:build/{lib_nm}.dll","/DLL","/DYNAMICBASE","/MACHINE:X64","/SUBSYSTEM:WINDOWS","/ERRORREPORT:none","/NOLOGO","/TLBID:1","/WX","/LTCG","/OPT:NOREF","/INCREMENTAL:NO","/RELEASE","build/"+b_nm+".lib"]+out_fl)):
				sys.exit(1)
		else:
			...
			if (util.execute(["link",f"/OUT:build/{lib_nm}.dll","/DLL","/DYNAMICBASE","/MACHINE:X64","/SUBSYSTEM:WINDOWS","/ERRORREPORT:none","/NOLOGO","/TLBID:1","/WX","/DEBUG","/INCREMENTAL:NO","/RELEASE",f"/PDB:build/sll_ext_{nm}.pdb","build/"+b_nm+".lib"]+out_fl)):
				sys.exit(1)
	else:
		if (r):
			util.log("  Compiling Library Files (Release Mode)...")
			...
			if (err):
				sys.exit(1)
			util.log("  Linking Library Files (Release Mode)...")
			if (util.execute(["gcc","-fdiagnostics-color=always","-shared","-fPIC","-fvisibility=hidden","-Wall","-O3","-Werror","-o",f"build/{lib_nm}.so","build/"+b_nm+".so"]+out_fl)):
				sys.exit(1)
			if (util.system!="darwin"):
				util.log("  Stripping Executable...")
				if (util.execute(["strip",f"build/{lib_nm}.so","-s","-R",".note.*","-R",".comment"])):
			...
			if (err):
				sys.exit(1)
			util.log("  Linking Library Files...")
			if (util.execute(["gcc","-fdiagnostics-color=always","-shared","-fPIC","-fvisibility=hidden","-Wall","-g","-O0","-Werror","-o",f"build/{lib_nm}.so","build/"+b_nm+".so"]+out_fl)):
				sys.exit(1)
