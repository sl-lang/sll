...
def build_sll(fl,v,r):
	...
	if (os.name=="nt"):
		...
		if (r):
			util.log("  Compiling Files (Release Mode)...")
			os.chdir("objects")
			if (util.wrap_output(["gcc","-fdiagnostics-color=always","-fPIC","-c","-fvisibility=hidden","-D","__SLL_COMPILATION__","-D","_GNU_SOURCE","-Wall","-O3","-Werror","-I","../../src/sll/include"]+e_fl+["../../"+e for e in fl]+["-lm"]).returncode!=0):
				os.chdir(cd)
				sys.exit(1)
			os.chdir("..")
			util.log("  Linking Files (Release Mode)...")
			...
			util.log("  Compiling Files...")
			os.chdir("objects")
			if (util.wrap_output(["gcc","-fdiagnostics-color=always","-fPIC","-c","-fvisibility=hidden","-D","__SLL_COMPILATION__","-D","DEBUG_BUILD","-D","_GNU_SOURCE","-Wall","-g","-O0","-Werror","-I","../../src/sll/include"]+e_fl+["../../"+e for e in fl]+["-lm"]).returncode!=0):
				os.chdir(cd)
				sys.exit(1)
			os.chdir("..")
			util.log("  Linking Files...")
...
def build_sll_test(r):
	...
	if (os.name=="nt"):
		...
		if (r):
			util.log("  Compiling & Linking Files (Release Mode)...")
			if (util.wrap_output(["gcc","-fdiagnostics-color=always","-Wall","-Werror","-O3","../tests/run_tests.c","-o","run_tests","-I",".","-I","../tests","-I","../src/sll/include"]+["objects/"+e for e in os.listdir("objects")]+["-lm","-ldl"]).returncode!=0):
				os.chdir(cd)
				sys.exit(1)
		else:
			util.log("  Compiling & Linking Files...")
			if (util.wrap_output(["gcc","-fdiagnostics-color=always","-Wall","-Werror","-g","-O0","../tests/run_tests.c","-o","run_tests","-I",".","-I","../tests","-I","../src/sll/include"]+["objects/"+e for e in os.listdir("objects")]+["-lm","-ldl"]).returncode!=0):
				os.chdir(cd)
				sys.exit(1)
	os.chdir(cd)
...
def build_sll_extension(fl,v,r):
	...
	if (os.name=="nt"):
		...
		if (r):
			...
			if (util.wrap_output(["gcc","-fdiagnostics-color=always","-fPIC","-c","-fvisibility=hidden","-Wall","-O3","-Werror","-I","../../src/ext/debug/include","-I","../"]+["../../"+e for e in fl]+["-lm"]).returncode!=0):
				os.chdir(cd)
				sys.exit(1)
			os.chdir("..")
			util.log("  Linking Library Files (Release Mode)...")
			...
			util.log("  Compiling Library Files...")
			os.chdir("objects_ext")
			if (util.wrap_output(["gcc","-fdiagnostics-color=always","-fPIC","-c","-fvisibility=hidden","-Wall","-g","-O0","-Werror","-I","../../src/ext/debug/include","-I","../"]+["../../"+e for e in fl]+["-lm"]).returncode!=0):
				os.chdir(cd)
				sys.exit(1)
			os.chdir("..")
			util.log("  Linking Library Files...")
