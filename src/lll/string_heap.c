...
__LLL_FUNC lll_string_t* lll_string_create(lll_string_length_t l){
	uint64_t sz=ALIGN(sizeof(lll_string_t)+(l+1)*sizeof(lll_char_t));
	...
	lll_string_t* o=(lll_string_t*)n;
	*((lll_string_length_t*)(&(o->l)))=l;
	o->rc=0;
	*((lll_heap_data_t*)(&(o->h)))=HEAP_SIGNATURE|(p<<8);
	o->c=0;
	return o;
}
...
__LLL_FUNC void lll_string_release(lll_string_t* s){
	...
	uint64_t sz=ALIGN(sizeof(lll_string_t)+(s->l+1)*sizeof(lll_char_t));
	if (sz<sizeof(memory_node_t)){
		sz=sizeof(memory_node_t);
	}
	sz+=GET_PADDING(s);
