...
uint32_t _get_object_size(const lll_object_t* o){
	...
	switch (LLL_GET_OBJECT_TYPE(o)){
		...
		case LLL_OBJECT_TYPE_STRING:
			return sizeof(lll_string_object_t)+eoff+((lll_string_object_t*)o)->ln*sizeof(char);
...
uint32_t _write_object(lll_output_data_stream_t* os,const lll_object_t* o){
	uint32_t eoff=0;
	while (o->t==LLL_OBJECT_TYPE_NOP){
		eoff+=sizeof(lll_object_type_t);
		o=LLL_GET_OBJECT_AFTER_NOP(o);
	...
	switch (LLL_GET_OBJECT_TYPE(o)){
		...
		case LLL_OBJECT_TYPE_FLOAT:
			...
			return sizeof(lll_float_object_t)+eoff;
		case LLL_OBJECT_TYPE_STRING:
			_write_integer(os,((lll_string_object_t*)o)->ln);
			LLL_WRITE_TO_OUTPUT_DATA_STREAM(os,(uint8_t*)(((lll_string_object_t*)o)->v),((lll_string_object_t*)o)->ln*sizeof(char));
			return sizeof(lll_string_object_t)+eoff+((lll_string_object_t*)o)->ln*sizeof(char);
		case LLL_OBJECT_TYPE_IDENTIFIER:
...
__LLL_IMPORT_EXPORT __LLL_RETURN lll_write_compiled_object(lll_output_data_stream_t* os,const lll_compilation_data_t* c_dt,uint8_t m,lll_error_t* e){
	...
	for (uint32_t i=0;i<c_dt->fp_dt.l;i++){
		...
		_write_integer(os,ifp->l);
		LLL_WRITE_TO_OUTPUT_DATA_STREAM(os,(uint8_t*)(ifp->fp),ifp->l*sizeof(char));
	}
	for (uint32_t i=0;i<LLL_MAX_SHORT_IDENTIFIER_LENGTH;i++){
		const lll_identifier_list_t* l=c_dt->i_dt.s+i;
		for (uint32_t j=0;j<l->l;j++){
			_write_integer(os,(l->dt+j)->sc);
			LLL_WRITE_TO_OUTPUT_DATA_STREAM(os,(uint8_t*)((l->dt+j)->v),(i+1)*sizeof(char));
		}
	}
	for (uint32_t i=0;i<c_dt->i_dt.ill;i++){
		const lll_identifier_t* k=*(c_dt->i_dt.il+i);
		_write_integer(os,k->sz);
		_write_integer(os,k->sc);
		LLL_WRITE_TO_OUTPUT_DATA_STREAM(os,(uint8_t*)(k->v),k->sz*sizeof(char));
	}
	for (uint32_t i=0;i<c_dt->im.l;i++){
		const lll_import_data_path_t* k=c_dt->im.dt+i;
		_write_integer(os,k->sz);
		if (k->sz!=UINT32_MAX){
			LLL_WRITE_TO_OUTPUT_DATA_STREAM(os,(uint8_t*)(k->nm),k->sz*sizeof(char));
		}
	}
